        ...
        <div class="col-lg-8 mb-4">
            <div class="card bg-white rounded shadow">
                <div class="position-relative overflow-hidden">
                    <img class="img-fluid w-100 rounded-top" src="{{ $berita->image }}" alt="{{ $berita->judul }}" style="height: 400px; object-fit: cover;">
                    <div class="position-absolute top-0 left-0 m-3 bg-dark text-white p-2 rounded">{{ date('d M Y', strtotime($berita->tgl)) }}</div>
                </div>
                <div class="p-4">
                    ...
                    <div class="d-flex justify-content-between">
                        <span class="text-muted">Diposting oleh {{ $berita->user->name }}</span>
                        <div class="d-flex">
            ...
            <div class="card bg-white p-4 rounded shadow mb-4">
                ...
                <div class="mb-3">
                    <a href="{{ route('berita.show', $news->id) }}" class="text-decoration-none text-dark">
                        <div class="d-flex">
                            <img src="{{ $news->image }}" alt="{{ $news->judul }}" class="me-3" style="width: 60px; height: 60px; object-fit: cover;">
                            <div>
                                <h5 class="mb-1">{{ $news->judul }}</h5>
                                <small class="text-muted"><i class="bi bi-calendar"></i> {{ date('d M Y', strtotime($news->tgl)) }}</small>
                            </div>
                        </div>
                    </a>
                </div>
