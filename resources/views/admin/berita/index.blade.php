              ...
              <div class="card shadow rounded" style="height: 100%;">
                ...
                <div class="card-footer">
                  <div class="d-flex justify-content-between">
                    <a href="{{ route('berita.show', $item->id) }}" class="btn btn-info btn-lg">Lihat</a>
                    <a href="{{ route('berita.edit', $item->id) }}" class="btn btn-warning btn-lg">Edit</a>
                    <form action="{{ route('berita.destroy', $item->id) }}" method="POST" style="display:inline;">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="btn btn-danger btn-lg" onclick="return confirm('Apakah Anda yakin ingin menghapus berita ini?')">Hapus</button>
                    </form>
                  </div>
                </div>
              </div>
